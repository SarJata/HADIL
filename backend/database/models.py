...
class HadilQueryHistory(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    natural_query = Column(String, nullable=False)
...
class HadilDatabaseInsight(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    database_name = Column(String, unique=True, index=True, nullable=False)
    generated_summary = Column(String, nullable=False)
    suggested_queries = Column(String, nullable=False) # JSON string
